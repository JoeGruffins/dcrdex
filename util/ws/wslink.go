...
const (
	...
	// disconnected link.
	ErrPeerDisconnected = util.ErrorKind("peer disconnected")

	ErrHandshake = util.ErrorKind("handshake error")
)
...
func NewWSLink(addr string, conn Connection, pingPeriod time.Duration, handler func(*msgjson.Message) *msgjson.Error, logger util.Logger) *WSLink {
	return &WSLink{
		addr:       addr,
		log:        logger,
		conn:       conn,
...
func NewConnection(w http.ResponseWriter, r *http.Request, readTimeout time.Duration) (Connection, error) {
	...
	if err != nil {
		...
		if errors.As(err, &hsErr) {
			err = util.NewError(ErrHandshake, hsErr.Error())
