...
func newMantle(name string) (*Mantle, error) {
	...
	waiter := util.NewStartStopWaiter(c)
	waiter.Start(ctx)
...
func (m *Mantle) initializeWallet(walletPW []byte, form *core.WalletForm, nCoins int, symbol string, maxFunds, minFunds uint64) (string, error) {
	...
	walletSymbol := util.BipIDSymbol(form.AssetID)
	...
	if nCoins != 0 {
		...
		if tkn := asset.TokenInfo(form.AssetID); tkn != nil {
			if err = send(util.BipIDSymbol(tkn.ParentID), addr, 1000e9); err != nil {
				if ignoreErrors && ctx.Err() == nil {
			...
			for {
				...
				m.log.Infof("%s fee balance not available yet. Trying again in 3 seconds", util.BipIDSymbol(tkn.ParentID))
			}
		}
