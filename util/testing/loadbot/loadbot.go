...
var (
	dcrID, _     = util.BipSymbolID(dcr)
	btcID, _     = util.BipSymbolID(btc)
	ethID, _     = util.BipSymbolID(eth)
	usdcID, _    = util.BipSymbolID(usdc)
	polygonID, _ = util.BipSymbolID(polygon)
	usdcpID, _   = util.BipSymbolID(usdcp)
	ltcID, _     = util.BipSymbolID(ltc)
	dashID, _    = util.BipSymbolID(dash)
	dogeID, _    = util.BipSymbolID(doge)
	dgbID, _     = util.BipSymbolID(dgb)
	firoID, _    = util.BipSymbolID(firo)
	bchID, _     = util.BipSymbolID(bch)
	zecID, _     = util.BipSymbolID(zec)
	zclID, _     = util.BipSymbolID(zcl)
	loggerMaker  *util.LoggerMaker
...
func run() error {
	...
	var found bool
	baseID, found = util.BipSymbolID(baseSymbol)
	if !found {
		return fmt.Errorf("base asset %q not found", baseSymbol)
	}
	quoteID, found = util.BipSymbolID(quoteSymbol)
	if !found {
	...
	loggerMaker, err = util.NewLoggerMaker(os.Stdout, logLevel)
	if err != nil {
