...
// AppServer is a single-client http and websocket server enabling a browser
...
// New is the constructor for a new AppServer. CustomSiteDir in the Config can
// be left blank, in which case a handful of default locations will be checked.
// This will work in most cases.
func New(cfg *Config) (*AppServer, error) {
	...
	if cfg.NoEmbed {
		...
		for _, dir := range []string{
			...
			absDir,
			filepath.Clean(filepath.Join(execPath, "../../appserver/site")),
		} {
			if dir == "" {
				continue
			}
		...
		if siteDir == "" {
			return nil, fmt.Errorf("no HTML template files found. "+
				...
				"or run bisonw from within the wallet/cmd/bisonw source workspace folder, or specify the"+
...
// from github.
func (w *AppServer) fetchLatestVersion(ctx context.Context) {
	var response struct {
...
// loaded from disk.
func (s *AppServer) buildTemplates(lang string) error {
	// Under the new UI, pages are served from a bundled SPA (newui/dist) and
	// there are no Go html/template files to build.
	if newUI {
		return nil
	}
...
// Addr gives the address on which AppServer is listening. Use only after
// Connect.
...
// service.
func (s *AppServer) isOnionRequest(r *http.Request) bool {
	if s.onion == "" {
		return false
	}
...
func (s *AppServer) authorize() string {
	b := make([]byte, 32)
	crand.Read(b)
...
// companionTokenTTL.
func (s *AppServer) authorizeCompanion() string {
	b := make([]byte, 32)
	crand.Read(b)
	token := hex.EncodeToString(b)
	zero(b)
...
func (s *AppServer) restoreCompanionFromHash(token string) bool {
...
// to match `AppServer.validAuthToken`. Requests through the Tor hidden
// service also require an active companion app pairing.
func (s *AppServer) isAuthed(r *http.Request) bool {
...
func (s *AppServer) getCachedPassword(key []byte, authToken string) ([]byte, error) {
	s.authMtx.Lock()
...
// in the request.
func (s *AppServer) getCachedPasswordUsingRequest(r *http.Request) ([]byte, error) {
	authToken := getAuthToken(r)
	if authToken == "" {
...
func (s *AppServer) cacheAppPassword(appPW []byte, authToken string) ([]byte, error) {
...
func (s *AppServer) isPasswordCached(r *http.Request) bool {
...
// websocket clients.
func (s *AppServer) readNotifications(ctx context.Context) {
...
// serveSPAIndex serves the bundled newui index.html. Under the new UI, the
// page routes do not render server-side templates; they deliver the SPA
// bootstrap document and the client performs its own routing.
func (s *AppServer) serveSPAIndex(w http.ResponseWriter) {
	var data []byte
	var err error
	if s.siteDir != "" {
		data, err = os.ReadFile(filepath.Join(s.siteDir, "dist", "index.html"))
	} else {
		data, err = staticSiteRes.ReadFile(path.Join(site, "dist", "index.html"))
	}
	if err != nil {
		log.Errorf("SPA index.html read error: %v", err)
	...
	if _, err := w.Write(data); err != nil {
		log.Errorf("error writing SPA index.html: %v", err)
	}
}
