...
func tBackend(t *testing.T, name string, isInternal bool, blkFunc func(string, error)) (*ExchangeWalletFullNode, *util.ConnectionMaster) {
	t.Helper()
...
func testWalletTxBalanceSync(t *testing.T, fromWallet, toWallet *ExchangeWalletFullNode) {
	...
	receipts, _, _, err := fromWallet.Swap(t.Context(), swaps)
	if err != nil {
	...
	_, out, _, err := toWallet.Redeem(t.Context(), &asset.RedeemForm{
