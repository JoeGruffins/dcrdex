...
Download and run setup as Administrator (privileges are required in order to set environment variables).  Open a **Command Prompt** (cmd.exe) as Administrator (not PowerShell), and run:
 ...
 ```batch
 cd %UserProfile%
 curl -O https://raw.githubusercontent.com/decred/dcrdex/master/wallet/cmd/bisonw-desktop/windows/setup-windows.cmd
...
This will also build the site bundle if `wallet/webserver/site/dist` does not exist.
...
```bash
docker build -t bisonw-desktop-windows -f wallet/cmd/bisonw-desktop/windows/Dockerfile .
```

To include XMR support:

```bash
docker build -t bisonw-desktop-windows --build-arg XMR=1 -f wallet/cmd/bisonw-desktop/windows/Dockerfile .
```
