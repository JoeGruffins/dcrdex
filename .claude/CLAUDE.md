...
cd util/evmrelay/cmd/evmrelay && go build -o evmrelay
...
- **`wallet/asset/`** — Per-chain wallet implementations (btc, eth, dcr, ltc, zec, xmr, bch, doge, firo, polygon, dash, dgb, zcl). Each implements `asset.Wallet` from `util/asset.go`. **Primary development area.**
...
- **Asset interface**: Every supported chain implements `asset.Wallet` from `util/asset.go`. Adding a new chain means implementing this interface and registering it under `wallet/asset/`.
