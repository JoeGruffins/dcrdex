...
func run(ctx context.Context) error {
	...
	if args[0] == "version" {
		...
		if err != nil {
			return fmt.Errorf("unable to check RPC server compatibility: failed to unmarshal result: %v", err)
		}

		if !dex.SemverCompatible(requiredRPCServerVersion, *verResp.RPCServerVer) {
			return fmt.Errorf("%s is not compatible with bisonw RPC server: required RPC server version - %s, bisonw RPC server version - %s",
