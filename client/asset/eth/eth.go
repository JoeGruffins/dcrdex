...
func (w *assetWallet) DeployContract(txData []byte) (contractAddr, txID string, err error) {
	if len(txData) == 0 {
		return "", "", errors.New("empty contract bytecode")
	}
	if w.assetID != w.baseChainID {
		return "", "", errors.New("contract deployment must use the base chain wallet")
	}
...
func (w *ETHWallet) TestContractGas(contractVer uint32, maxSwaps int, tokenAssetIDs []uint32) ([]*asset.GasTestResult, error) {
	if contractVer != 1 {
	...
	results := []*asset.GasTestResult{baseResult}
	...
	for _, tokenAssetID := range tokenAssetIDs {
		...
		var tokenResult *asset.GasTestResult
		err := w.withTokenContractor(tokenAssetID, contractVer, func(tc tokenContractor) error {
			tg := w.gases(contractVer)
			var testErr error
			tokenResult, testErr = testContractGasForAsset(ctx, cl, tc, contractVer, maxSwaps, tg, token.AtomicToEVM,
				true /* isToken */, log, tc, w.defaultProviders, w, tokenAssetID, dex.BipIDSymbol(tokenAssetID))
			return testErr
		})
		if err != nil {
			results = append(results, &asset.GasTestResult{
...
func testGaslessRedeem(
	...
	if len(rawVerification) > 0 {
		...
		result.GaslessRedeemCall = recommendedGas(rawCall[0])
		if len(rawVerification) > 1 {
			...
			result.GaslessRedeemCallAdd = recommendedGas(avgDiff(rawCall))
		}
...
// avgDiff calculates the average difference between consecutive elements.
