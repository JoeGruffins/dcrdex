...
func TestCommandUsage(t *testing.T) {
	...
	resNoPW, _ := commandUsage("send", false)
	resWithPW, _ := commandUsage("send", true)
	if strings.Contains(resNoPW, "appPass") {
		t.Fatal("commandUsage(send, false) should not contain appPass")
	}
	if !strings.Contains(resWithPW, "appPass") {
		t.Fatal("commandUsage(send, true) should contain appPass")
	}
}
...
func TestRouteExists(t *testing.T) {
	...
	if !RouteExists("send") {
		t.Fatal("RouteExists returned false for 'send'")
	}
...
func TestParamType(t *testing.T) {
	...
	pt := ParamType("send")
	if pt == nil {
		t.Fatal("ParamType returned nil for 'send'")
	}
	if pt != reflect.TypeFor[SendParams]() {
		t.Fatalf("ParamType('send') = %v, want %v", pt, reflect.TypeFor[SendParams]())
	}
...
func TestExportedReflectFields(t *testing.T) {
	pt := ParamType("send")
	if pt == nil {
		t.Fatal("no param type for send")
	}
	fields := ReflectFields(pt)
	if len(fields) == 0 {
		t.Fatal("ReflectFields returned no fields for SendParams")
	}
	...
	// SendParams should have appPass (password) and address.
	if !nameSet["appPass"] {
		t.Fatal("appPass not found in exported ReflectFields")
	}
	if !nameSet["address"] {
		t.Fatal("address not found in exported ReflectFields")
	}
	...
	for _, f := range fields {
		...
		if f.JSONName == "address" && f.IsPassword {
			t.Fatal("address should not be marked as password")
		}
	}
}
