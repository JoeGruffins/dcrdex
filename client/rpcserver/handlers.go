...
var routeInfos = map[string]routeInfo{
	...
	testContractGasRoute: {
		...
		fieldDescs: map[string]string{
			"appPass":  descAppPass,
			"chains":   `List of base chain symbols, e.g. ["eth", "polygon", "base"].`,
			"tokens":   `Optional list of token symbols, e.g. ["usdc.eth", "usdc.polygon"]. Tokens are tested on their parent chain.`,
    ...
    Test ETH base chain gas with defaults:
      bwctl testcontractgas '{"appPass":"abc","chains":["eth"]}'

    Test with tokens:
      bwctl testcontractgas '{"appPass":"abc","chains":["eth"],"tokens":["usdc.eth"],"maxSwaps":3}'`
		},
	},
...
func handleDeployContract(s *RPCServer, msg *msgjson.Message) *msgjson.ResponsePayload {
	...
	case params.ContractVer != nil:
		...
		var err error
		txData, err = eth.BuildDeployTxData(*params.ContractVer, tokenAddr)
		if err != nil {
			resErr := msgjson.NewError(msgjson.RPCArgumentsError, "error building contract data: %v", err)
			return createResponse(deployContractRoute, nil, resErr)
		}
	default:
	...
	results, err := s.core.DeployContract(params.AppPass, assetIDs, txData)
	if err != nil {
